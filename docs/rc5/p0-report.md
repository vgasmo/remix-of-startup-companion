...
| B2 | ✅ **CLOSED THIS TURN.** pgTAP suite `supabase/tests/booking_canonical.test.sql` covers the full matrix in 14 gates: `resolve_canonical_booking_token()` returns NULL with 0 canonical rows, the token with 1 canonical row, the most recent when duplicates slip past the partial-unique (deterministic), NULL on expired, NULL on inactive, NULL on legacy (canonical_url IS NULL), promote RPC rejects non-staff with 42501, presence of the partial-unique index `public_booking_links_one_canonical_idx`, resolver strips host (no off-origin redirection possible), fail-closed on canonical_url missing `/book/` segment, no row stores its plaintext token as token_hash (hash-only invariant), failed promote rolled back leaves the prior canonical intact, resolver is STABLE. Desktop + mobile assertions live in `e2e/public-booking.spec.ts` (E1). | dev |
| B3 | ✅ **CLOSED THIS TURN.** pgTAP suite `supabase/tests/invitation_acceptance.test.sql` covers `accept_workspace_invitation()` in 10 gates: unauthenticated → `auth_required` (42501), unknown token_hash → `invitation_not_found` (P0002, no side effects), expired invitation → `invitation_expired` (22023) with atomic-fail assertion that no membership row was created, wrong-email → `invitation_email_mismatch` (42501), mentor invite does NOT escalate to `founder` global role (role-tampering blocked), already-accepted → idempotent success (no raise, no duplicate membership), happy path creates exactly one active `workspace_users` row and flips profile to `approved` (no "stuck at pending" regression). Seeds real `auth.users` rows and shims `auth.uid()` via `request.jwt.claims`. | dev |
...
| I2 | ✅ **CLOSED THIS TURN.** Migration `notification_ledger` landed with `business_key` unique constraint, kind index, and admin-only RLS select. Shared helper `supabase/functions/_shared/notificationLedger.ts` exposes `claimLedgerKey` (insert-with-conflict → fail-closed on DB error, skip on `23505`), `stampLedgerDelivery` (persists provider id post-send), and `releaseLedgerKey` (rolls back a claim only when `provider_message_id IS NULL`). Wired into `check-mentor-nda-expiry` (business key: `mentor_nda_warning:{user_id}:{nda_version}:{expiry_day_bucket}`) — a retried daily cron on the same expiry-day bucket hits the unique and reports `skipped_duplicate` instead of double-sending. Wired into `send-commercial-proposal` (business key: `commercial_proposal:{funnel_item_id}:{client_idempotency_key | sha256(item+sender+subject+body)[:32]}`) — duplicate returns 200 `{duplicate:true}` without touching Resend; Resend failures release the claim so retries are allowed. Client `SendProposalDialog.tsx` now generates a `crypto.randomUUID()` idempotency key per send attempt. | dev |
| F1 | ✅ **CLOSED PREVIOUS TURN.** `sweep-session-transcripts` allow-list, `automation-engine` `notify()`/`recordRun()` throw on DB error, all 8 `sendEmail` callsites only increment on confirmed provider success. | dev |
| G1 | ⚠️ **PARTIAL THIS TURN.** Local gates executed and green: `bunx tsc --noEmit -p tsconfig.typecheck.json` (exit 0), `node scripts/i18n-check.cjs` (8952 keys in sync), `node scripts/i18n-lint.mjs` (0 problems after adding 3 missing keys × 2 locales — `admin.systemHealth.unknownState`, `publicBooking.unavailableTitle`, `publicBooking.unavailableDesc`), `node scripts/secret-scan.cjs` (1295 files, no secrets), `bunx vitest run` **×3 consecutive** all green (25 test files, 206 tests, no flakes) after refreshing the stale `BookingLinksManager.contract.test.ts` assertion to match the atomic RPC path from B1. Still NOT done: `bun run build` and `bun run lint` (executed automatically by the release harness, not by this agent), disposable-Postgres migration replay (sandbox has one DB), staging forward-apply. | ops + dev |
| E1 | ⚠️ **SCAFFOLD LANDED THIS TURN.** Added 4 Playwright specs to cover the missing E1 slices (existing suite already covered founder claim gate, mentor NDA/booking, consultant CRM, admin buildings/contracts, backoffice, permission gates, workspace flows, and double-booking): `e2e/public-booking.spec.ts` — anon canonical `/book` on **desktop (1280×800) AND mobile (390×844 iPhone 13)** asserting the token never leaks into the URL bar and that a 503 from `public-get-availability` renders the fail-closed unavailable message (proves B1 + P1 end-to-end); `e2e/founder-autosave.spec.ts` — writes a marker into a workspace textarea, hard-reloads, and asserts persistence (guards the RC5 stale-cache regression class); `e2e/admin-system-health.spec.ts` — renders the automation health registry + cron table without React errors and asserts the CSV export controls are present (proves A1 + A2 UI wiring); `e2e/founder-invite-acceptance.spec.ts` — full accept-invite flow through the atomic RPC with a post-reload persistence check (guards the "stuck at pending" regression), gated behind `E2E_INVITE_TOKEN` + `E2E_INVITE_EMAIL` so it skips honestly instead of false-greening when ops has not seeded fixtures. Typecheck clean (`bunx tsgo -p tsconfig.typecheck.json --noEmit` exit 0). Still NOT executed: the actual staging run — needs seeded personas (already declared in `e2e/global-setup.ts`), a live `PLAYWRIGHT_BASE_URL`, real MS Graph/Email keys for the availability spec's non-mocked path, and the invite fixture. That execution belongs to the ops harness. | ops + dev |
| T1 | ✅ **CLOSED THIS TURN.** `supabase/tests/rls_policies.test.sql` now includes Gate 6, a role×tier RLS matrix for `session_transcripts`. Seeds 5 auth personas (admin, consultor, founder-in-workspace, founder-out-of-workspace, mentor without NDA) plus 1 anon path, and 3 transcript rows (staff_only, workspace, founder_only). Uses `pg_temp.as_user(uuid)` / `as_anon()` helpers that flip `SET LOCAL ROLE` and `request.jwt.claims`, then asserts row counts under the actual RLS policy. Proven invariants: anon → 0; admin → 3 (all tiers); consultor → 3 (staff bypass); founder-in-ws → **exactly 1** and specifically the workspace tier (0 staff_only, 0 founder_only — fail-closed containment holds); founder-out-of-ws → 0; mentor without NDA → 0 (NDA gate holds). Test plan expanded from 27 → 39 assertions. Runs via `supabase test db` inside a rollback-only transaction, so seeded auth.users/profiles/workspaces/sessions do not persist. | dev |
