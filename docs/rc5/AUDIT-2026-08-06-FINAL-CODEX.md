...
| P0-C | past-meeting RPC auth/idempotency/attendee gaps | **FIXED + PROVEN**. pgTAP executed for the first time via `scripts/rc5/local-pg-harness.sh` (disposable local PG, full 443-migration replay). It exposed a real defect the earlier hardening missed: replaying a known `command_id` with a **tampered payload** returned the original session instead of failing closed. Migration 2026-08-06 adds `sessions.command_fingerprint` (SHA-256 over actor + all payload fields) and raises `42501 command_fingerprint_mismatch`. Suite: **22/22 pass**. | `PROVEN` |
