...
| F2 | Mentor lifecycle + operational reporting | `FIXED IN SOURCE / RUNTIME NOT PROVEN` | Draft `docs/rc5/drafts/2026-07-22_batch-f2_sessions_view.sql` adds security-invoker `v_sessions_operational` (no consultant+mentor double-count; `effective_minutes` prefers Graph-measured over slot) and `transition_mentor_booking_atomic` (SECURITY DEFINER, FOR UPDATE, terminal-state guard, idempotent replay, session-row propagation on cancel/no-show). pgTAP `supabase/tests/mentor_booking_transition.test.sql`. |
| F3 | CRM import + reconciliation | `FIXED IN SOURCE / RUNTIME NOT PROVEN` | Draft `docs/rc5/drafts/2026-07-22_batch-f3_import_dedupe.sql` adds IMMUTABLE `normalize_ident`, STORED generated columns `funnel_items.{nif,email,company}_normalized`, partial unique indexes on active leads, `crm_import_conflicts` staff queue + RLS, `crm_lead_import_batches.lifecycle_state` (`draft/committing/committed/partial_needs_review/failed`) + `finalize_crm_import_batch`. pgTAP `supabase/tests/crm_import_dedupe.test.sql`. |
| F4 | Programme publication | `FIXED IN SOURCE / RUNTIME NOT PROVEN` | Draft `docs/rc5/drafts/2026-07-22_batch-f4_publish_atomic.sql` adds `program_publish_snapshots` audit + `serialize_program_tree` + `publish_program_atomic` (staff guard, FOR UPDATE on target + previous-active peer, snapshot-before-mutation, pointer swap, post-swap re-serialize with 23514 on previous-active tree breakage). Preserves both acceleration and incubation trees. pgTAP `supabase/tests/program_publish_atomic.test.sql`. |
| F5 | Business Plan / Financial Plan assistants | `FIXED IN SOURCE / RUNTIME NOT PROVEN` | Draft `docs/rc5/drafts/2026-07-22_batch-f5_financial_scenario.sql` adds `financial_model_versions.command_fingerprint` + partial unique `(session_id, command_fingerprint)` and `save_financial_scenario_atomic` (workspace-access check, FOR UPDATE session lock, idempotent replay short-circuit, atomic assumption-KV snapshot for exact clones). Deferred (tracked in `docs/rc5/failing-repros/batch-f5.md`): localStorage autosave, scoring dimensions UI, deterministic Excel round-trip assertion, mobile scroll fixes, facts-vs-assumptions labelling. pgTAP `supabase/tests/financial_scenario_atomic.test.sql`. |
