-- RC5 Batch F1 — Public First-Contact Booking hardening (forward-only, additive, idempotent).
--
-- Fixes:
--   1. Europe/Lisbon wall-clock pin in commit_first_contact_booking_atomic (was casting via session TZ).
--   2. Deep-merge metadata_json (was shallow || merge — nested keys clobbered).
--   3. Composite unique index (link_id, submitter_email_normalized, booking_slot_utc) at DB level
--      — the previous idempotency_key-only index did not protect against a client that reuses the
--      same link+email+slot with a fresh nonce.
--   4. claim_first_contact_outbox_batch / mark_first_contact_outbox_* RPCs — authoritative delivery
--      state for the retry worker.
--
-- No data mutation. Safe on production. Held as draft.

-- ---------- 1. Europe/Lisbon + deep-merge in commit RPC ----------
...
DECLARE
  ...
  -- Explicit Europe/Lisbon pin: the wall-clock the caller submitted is Lisbon local,
  -- not server-local. AT TIME ZONE 'Europe/Lisbon' converts the naive timestamp into
  -- the correct UTC instant across DST boundaries.
...
BEGIN
  ...
  -- Enrich metadata with canonical UTC + link binding so the composite unique
  -- index can enforce (link_id, email, slot_utc) at the DB level.
  v_enriched_meta := coalesce(p_metadata,'{}'::jsonb) || jsonb_build_object(
    ...
    'submitter_email_normalized', v_email,
    'link_id', v_link_id
  );
  ...
  -- 1b) Composite dedupe (link_id, email, slot_utc) — protects against
  -- distinct idempotency keys targeting the same physical slot.
  ...
  IF v_existing_id IS NOT NULL THEN
    UPDATE public.funnel_items
       SET first_contact_at = v_first_contact_at,
           ...
           -- Deep merge preserves nested routing_decision.trace and other structured children.
  ...
  BEGIN
    INSERT INTO public.funnel_items (
      ...
      v_first_contact_at, v_enriched_meta
    )
    RETURNING id INTO v_new_id;
  EXCEPTION WHEN unique_violation THEN
    -- Concurrent insert won the race against the idempotency index — reuse the winner.
    SELECT id INTO v_existing_id
    FROM public.funnel_items
    WHERE metadata_json->>'idempotency_key' = p_idempotency_key
    LIMIT 1;
...
-- ---------- 2. Composite unique index (link_id + email + slot_utc) ----------
-- Fail-closed: the DB rejects a second booking for the same (link, email, slot)
-- even when a client cycles the idempotency_key.
CREATE UNIQUE INDEX IF NOT EXISTS uniq_funnel_items_link_email_slot_utc
  ON public.funnel_items (
    ((metadata_json->>'link_id')),
    ((metadata_json->>'submitter_email_normalized')),
    ((metadata_json->>'booking_slot_utc'))
  )
  WHERE metadata_json ? 'link_id'
    AND metadata_json ? 'submitter_email_normalized'
    AND metadata_json ? 'booking_slot_utc';


-- ---------- 3. Outbox retry worker RPCs ----------
-- claim_first_contact_outbox_batch leases up to N pending/failed rows past next_attempt_at
-- using SKIP LOCKED so parallel workers do not double-dispatch. Marks them in_progress.
CREATE OR REPLACE FUNCTION public.claim_first_contact_outbox_batch(
  p_batch_size int DEFAULT 25,
  p_lease_seconds int DEFAULT 120
)
RETURNS SETOF public.first_contact_outbox
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
BEGIN
  ...
  WITH claimed AS (
    SELECT id
    FROM public.first_contact_outbox
...
CREATE OR REPLACE FUNCTION public.mark_first_contact_outbox_failed(
  p_id uuid,
  p_error text,
