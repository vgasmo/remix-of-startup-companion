-- Batch F3 — CRM import + reconciliation
-- Forward-only, additive, idempotent. Held as draft.
--
-- Normalized dedupe keys + human conflict queue + all-or-partial batch
-- semantics. Does not modify existing importer RPC signatures.

BEGIN;

-- 1. Deterministic normalizer (immutable) ----------------------------------
...
AS $$
  SELECT CASE
    ...
    ELSE lower(
      -- strip diacritics via unaccent-equivalent regex; keep letters + digits
      regexp_replace(
        translate(
          btrim(_v),
...
-- 2. Normalized generated columns + partial unique indexes ------------------
ALTER TABLE public.funnel_items
  ADD COLUMN IF NOT EXISTS nif_normalized text
    GENERATED ALWAYS AS (public.normalize_ident(nif)) STORED;

ALTER TABLE public.funnel_items
  ADD COLUMN IF NOT EXISTS email_normalized text
    GENERATED ALWAYS AS (public.normalize_ident(email)) STORED;

ALTER TABLE public.funnel_items
  ADD COLUMN IF NOT EXISTS company_normalized text
    GENERATED ALWAYS AS (public.normalize_ident(company)) STORED;

CREATE UNIQUE INDEX IF NOT EXISTS uniq_funnel_items_nif_active
  ON public.funnel_items (nif_normalized)
  WHERE nif_normalized IS NOT NULL AND stage <> 'archived';

CREATE UNIQUE INDEX IF NOT EXISTS uniq_funnel_items_email_active
...
CREATE POLICY crm_import_conflicts_staff_read
  ON public.crm_import_conflicts FOR SELECT
  TO authenticated
...
CREATE POLICY crm_import_conflicts_staff_write
  ON public.crm_import_conflicts FOR UPDATE
  TO authenticated
  USING (public.has_role(auth.uid(), 'admin') OR public.has_role(auth.uid(), 'backoffice'))
...
-- Helper to finalize a batch after commit.
...
BEGIN
  ...
  UPDATE public.crm_lead_import_batches
     SET lifecycle_state = v_new_state,
         updated_at = now()
   WHERE id = p_batch_id;
