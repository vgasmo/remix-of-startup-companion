-- ============================================================================
-- Batch D — Monthly Founder Pulse OFF-first guardrails (forward-only DRAFT)
-- ----------------------------------------------------------------------------
-- Status: DRAFT — not applied. Applies to staging alongside the pgTAP suite
-- `supabase/tests/founder_pulse_off_state.test.sql`.
--
-- Enforces:
--   1. Server-side feature-flag gate at every entry point.
--   2. Eligibility restricted to active founders (`is_account_active`).
--   3. Unique key `(cycle_id, workspace_id, respondent_id, channel)` on
--      notification_attempts (via generated columns) — dedupe no longer
--      relies on client_command_id string uniqueness alone.
--   4. `notification_attempts.state` check constraint.
--   5. Retention: anonymize responses older than 18 months.
-- ============================================================================
BEGIN;

-- 1. Helper: read a global flag safely from SECURITY DEFINER code -----------
...
AS $$
  SELECT COALESCE(
    (SELECT enabled FROM public.feature_flags
      WHERE key = p_key AND scope = 'global' LIMIT 1),
    false
  );
$$;
...
-- 2. Ensure the founder_monthly_pulse flag exists and defaults OFF -----------
INSERT INTO public.feature_flags (key, enabled, scope, description)
SELECT 'founder_monthly_pulse', false, 'global',
       'Monthly Founder Pulse — kill switch. OFF by default.'
WHERE NOT EXISTS (
  SELECT 1 FROM public.feature_flags
   WHERE key = 'founder_monthly_pulse' AND scope = 'global'
);

-- 3. Wrap the cron entry point in a flag guard -------------------------------
-- `open_and_notify_monthly_founder_pulse_cycles` is the sole cron surface.
...
BEGIN
  ...
  FOR v_cycle IN
    SELECT id FROM public.founder_pulse_cycles
     WHERE status = 'open'
  ...
  RETURN jsonb_build_object(
    'ok', true,
    'opened', v_opened,
    'enqueued', v_enqueued_total,
    'period_month', to_char(date_trunc('month', now())::date, 'YYYY-MM-DD')
  );
END;
$$;
...
-- 4. Belt-and-braces flag guard on cycle opener too --------------------------
...
-- 5. Eligibility hardening on enqueue: active founder + active account -------
--    Replaces the existing SECURITY DEFINER function; keeps the same
--    signature so all callers (rc5_run_batch_e, edge functions) work.
...
-- 6. Structural dedup: unique (cycle, workspace, user, channel) --------------
-- Generated columns extract the identity tuple from metadata so a compromised
-- caller cannot bypass dedup by rewriting client_command_id.
ALTER TABLE public.notification_attempts
  ADD COLUMN IF NOT EXISTS pulse_cycle_id UUID
    GENERATED ALWAYS AS ((metadata->>'cycle_id')::uuid) STORED,
  ADD COLUMN IF NOT EXISTS pulse_workspace_id UUID
    GENERATED ALWAYS AS ((metadata->>'workspace_id')::uuid) STORED,
  ADD COLUMN IF NOT EXISTS pulse_respondent_id UUID
    GENERATED ALWAYS AS ((metadata->>'user_id')::uuid) STORED;
...
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE public.notification_attempts
      ADD CONSTRAINT notification_attempts_state_chk
      CHECK (state IN (
        'queued','leased','delivered','failed_retryable','failed_terminal'
      )) NOT VALID;
    -- NOT VALID so a stray legacy row doesn't block deploy; a follow-up job
    -- validates once the backfill lands.
  END IF;
END $$;

-- 8. Retention: anonymize pulse responses older than 18 months ---------------
