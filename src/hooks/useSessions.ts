  ...
  return useMutation({
    ...
    onSuccess: async (result) => {
      ...
      supabase.functions.invoke('recompute-health-scores', {
        body: { workspaceId },
      }).catch((err) => logger.warn('recompute_health_score_failed', { workspaceId }, err));
      ...
      if (result.needsSync) {
        syncOutlookCalendar({
          ...
          action: 'update',
          workspaceId,
        }).catch((err) => logger.warn('outlook_sync_update_failed', { workspaceId, sessionId: result.session.id }, err));
        ...
        (async () => {
          ...
          try {
            ...
            logger.warn('session_reschedule_lookup_failed', { workspaceId }, err);
          }
          ...
          }).catch((err) => logger.warn('teams_notify_reschedule_failed', { workspaceId }, err));
        })().catch((err) => logger.warn('session_reschedule_notify_wrapper_failed', { workspaceId }, err));
      }
    },
  });
}
  ...
  return useMutation({
    mutationFn: async (sessionId: string) => {
      ...
      await syncOutlookCalendar({
        ...
        action: 'delete',
        workspaceId,
      }).catch((err) => logger.warn('outlook_sync_delete_failed', { workspaceId, sessionId }, err));
