      ...
      `}>
        <div className="flex items-start justify-between gap-2">
          ...
          {canWrite && (item.source_deliverable_key === null || isStaff) && (
            <Button variant="ghost" size="icon" className="h-6 w-6 opacity-0 group-hover:opacity-100 shrink-0" onClick={()= aria-label="Delete"> onDelete(item)} aria-label={t('common.delete')}>
            ...
            {deliverables.map(d => (
              <div key={d.id} className="flex items-center gap-2 text-xs group/del">
                ...
                {d.completed_at ? (
                  ...
                  <Button variant="ghost" size="icon" className="h-5 w-5 shrink-0 opacity-0 group-hover/del:opacity-100" onClick={()= aria-label="Confirm"> onCompleteDeliverable(d.id, item.id)} aria-label={t('common.confirm')}>
