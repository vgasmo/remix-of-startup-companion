                ...
                <div key={item.id} className="flex items-center gap-3 p-3 rounded-lg border bg-card hover:bg-muted/30 transition-colors">
                  ...
                  {canWrite && (
                    <div className="flex gap-1">
                      <Button variant="ghost" size="icon" onClick={()= aria-label="Hide"> handleToggleVisibility(item)} title={t('dataroom.toggleVisibility')} aria-label={t('common._iconHide')}>
                        {item.visibility === 'investors' ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                      </Button>
                      <Button variant="ghost" size="icon" onClick={()= aria-label="Delete"> handleDeleteItem(item.id)} className="text-destructive" aria-label={t('common.delete')}>
