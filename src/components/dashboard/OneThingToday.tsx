  ...
  const recommendation = useMemo<RecommendedAction | null>(() => {
    ...
    if (overdueActions.length > 0) {
      ...
      candidates.push({
        ...
        link: `/workspace/${workspace.id}?tab=milestones-actions&sub=actions&highlight=${oldest.id}`,
    ...
    if (pendingActions.length > 0 && candidates.length === 0) {
      ...
      candidates.push({
        ...
        link: `/workspace/${workspace.id}?tab=milestones-actions&sub=actions&highlight=${next.id}`,
        priority: 30,
        icon: CheckCircle2,
        variant: 'default',
      });
