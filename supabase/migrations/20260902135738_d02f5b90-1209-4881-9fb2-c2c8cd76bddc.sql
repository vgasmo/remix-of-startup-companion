  ...
  ws AS (
    ...
    WHERE w.status IN ('imported_unclaimed','claimed','pending','active')
      ...
      AND (p_tag_id IS NULL OR EXISTS (
        SELECT 1 FROM public.workspace_tags wt WHERE wt.workspace_id = w.id AND wt.tag_id = p_tag_id))
      AND (p_category_id IS NULL OR EXISTS (
        SELECT 1 FROM public.workspace_tags wt
        JOIN public.tags tg ON tg.id = wt.tag_id
        WHERE wt.workspace_id = w.id AND tg.category_id = p_category_id))
      AND (p_needs_attention IS NOT TRUE OR (
        COALESCE(w.health_score_override::text, w.health_score::text) IN ('critical','at_risk')
        OR EXISTS (
          SELECT 1 FROM public.action_items ai
          WHERE ai.workspace_id = w.id AND ai.status <> 'completed'
            AND ai.due_date IS NOT NULL AND ai.due_date < CURRENT_DATE)
        OR NOT EXISTS (
          SELECT 1 FROM public.kpi_values kv
          WHERE kv.workspace_id = w.id
            AND kv.period_month >= date_trunc('month', CURRENT_DATE)::date)
      ))
  ),
  fi AS (
    ...
    WHERE f.linked_workspace_id IS NULL AND f.stage::text NOT IN ('lost','disqualified','contracted')
      ...
      AND p_tag_id IS NULL AND p_category_id IS NULL AND p_needs_attention IS NOT TRUE
...
GRANT EXECUTE ON FUNCTION public.list_ecosystem_items_v2(uuid, text, text, uuid, boolean, text, timestamptz, uuid, integer, uuid, uuid, text, text, uuid, uuid, boolean) TO authenticated, service_role;