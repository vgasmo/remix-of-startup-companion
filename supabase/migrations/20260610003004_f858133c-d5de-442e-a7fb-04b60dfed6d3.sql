-- Server-paginated workspaces search (for MyWorkspaces detailed list view)
...
GRANT EXECUTE ON FUNCTION public.search_workspaces_paged(text[], uuid, text, uuid, public.startup_stage, public.health_score, public.workspace_priority, text, int, int) TO authenticated;
GRANT EXECUTE ON FUNCTION public.search_workspaces_paged(text[], uuid, text, uuid, public.startup_stage, public.health_score, public.workspace_priority, text, int, int) TO service_role;