...
AS $function$
  ...
  ws AS (
    SELECT
      ...
      -- Real per-workspace activity. w.updated_at is unreliable here because
      -- bulk recomputations touch every row and made all startups share one date.
      COALESCE(act.last_activity_at, w.created_at) AS last_activity_at,
    ...
    LEFT JOIN LATERAL (
      SELECT max(ts) AS last_activity_at FROM (
        SELECT max(al.created_at) AS ts FROM public.activity_log al WHERE al.workspace_id = w.id
        UNION ALL
        SELECT max(we.created_at) FROM public.workspace_engagement_events we WHERE we.workspace_id = w.id
        UNION ALL
        SELECT max(GREATEST(se.updated_at, se.scheduled_at)) FROM public.sessions se WHERE se.workspace_id = w.id
        UNION ALL
        SELECT max(ai.updated_at) FROM public.action_items ai WHERE ai.workspace_id = w.id
        UNION ALL
        SELECT max(m.updated_at) FROM public.milestones m WHERE m.workspace_id = w.id
        UNION ALL
        SELECT max(kv.updated_at) FROM public.kpi_values kv WHERE kv.workspace_id = w.id
        UNION ALL
        SELECT max(d.updated_at) FROM public.documents d WHERE d.workspace_id = w.id
      ) x
    ) act ON true
