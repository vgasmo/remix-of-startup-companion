-- Drift closure: production already carries the staff-only guard on
-- issue_contract_signing_grant, but the tracked migration tree did not.
-- Re-declared verbatim so replays (and the pgTAP harness) match production.
