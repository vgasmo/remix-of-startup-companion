...
BEGIN
  BEGIN
    INSERT INTO auth.users(id, email) VALUES
      (admin_id,        'admin@rc5-d.test'),
      (consultor_id,    'cons@rc5-d.test'),
      (founder_ok_id,   'ok@rc5-d.test'),
      (founder_pend_id, 'pend@rc5-d.test'),
      (founder_susp_id, 'susp@rc5-d.test'),
      (founder_off_id,  'off@rc5-d.test'),
      (stranger_id,     'stg@rc5-d.test'),
      (mentor_nda_id,   'mnda@rc5-d.test'),
      (mentor_nonda_id, 'mnn@rc5-d.test');

    -- handle_new_user trigger already inserted profiles; adjust account_status.
    UPDATE public.profiles SET account_status='approved'  WHERE id=admin_id;
    UPDATE public.profiles SET account_status='approved'  WHERE id=consultor_id;
    UPDATE public.profiles SET account_status='approved'  WHERE id=founder_ok_id;
    UPDATE public.profiles SET account_status='pending'   WHERE id=founder_pend_id;
    UPDATE public.profiles SET account_status='suspended' WHERE id=founder_susp_id;
    UPDATE public.profiles SET account_status='approved'  WHERE id=founder_off_id;
    UPDATE public.profiles SET account_status='approved'  WHERE id=stranger_id;
    UPDATE public.profiles SET account_status='approved'  WHERE id=mentor_nda_id;
    UPDATE public.profiles SET account_status='approved'  WHERE id=mentor_nonda_id;
