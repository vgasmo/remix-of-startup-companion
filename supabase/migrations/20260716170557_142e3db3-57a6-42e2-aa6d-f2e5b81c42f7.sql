...
DROP FUNCTION IF EXISTS public.list_ecosystem_items_v2(uuid, text, text, uuid, boolean, text, timestamptz, uuid, int);
...
AS $function$
  WITH ws_active_contract AS (
    ...
    WHERE sc.status = 'active' AND sc.terminated_at IS NULL
    ORDER BY sc.workspace_id, sc.start_date DESC NULLS LAST
  ),
  ws_current_space AS (
  ...
  ws AS (
    SELECT
      ...
      it.id   AS incubation_type_id,
      it.name AS incubation_type_name,
      CASE
      ...
      cs.building_id,
      cs.building_name,
      cs.office_space_id AS space_id,
      cs.space_name
    ...
    SELECT
      ...
      NULL::uuid AS incubation_type_id,
      NULL::text AS incubation_type_name,
...
REVOKE ALL ON FUNCTION public.list_ecosystem_items_v2(
  uuid, text, text, uuid, boolean, text, timestamptz, uuid, integer, uuid, uuid, text, text
) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.list_ecosystem_items_v2(
  uuid, text, text, uuid, boolean, text, timestamptz, uuid, integer, uuid, uuid, text, text
) TO authenticated, service_role;

COMMENT ON FUNCTION public.list_ecosystem_items_v2 IS
  'Ecosystem listing v2 — returns real incubation type, modality, building and space per workspace, plus filter parameters for each.';
...
AS $function$
  ...
  ws_current_space AS (
    SELECT DISTINCT ON (sa.workspace_id)
      sa.workspace_id, sa.office_space_id, os.building_id
    FROM public.space_allocations sa
    LEFT JOIN public.office_spaces os ON os.id = sa.office_space_id
    WHERE sa.end_date IS NULL OR sa.end_date > CURRENT_DATE
    ORDER BY sa.workspace_id, sa.start_date DESC NULLS LAST
  ...
  SELECT jsonb_build_object(
    ...
    'by_incubation_type', (
      SELECT COALESCE(jsonb_agg(jsonb_build_object(
        'id', incubation_type_id, 'name', incubation_type_name, 'count', ct
      )), '[]'::jsonb)
      FROM (
        SELECT incubation_type_id, incubation_type_name, COUNT(*) AS ct
        FROM filtered
        GROUP BY incubation_type_id, incubation_type_name
        ORDER BY ct DESC NULLS LAST
      ) x
    ),
    'by_tier', (
      SELECT COALESCE(jsonb_object_agg(coalesce(startup_category,'unclassified'), ct), '{}'::jsonb)
      FROM (SELECT startup_category, COUNT(*) AS ct FROM filtered GROUP BY startup_category) x
    ),
    'by_consultant', (
      SELECT COALESCE(jsonb_agg(jsonb_build_object(
        'consultant_id', owner_id, 'count', ct
      )), '[]'::jsonb)
      FROM (SELECT owner_id, COUNT(*) AS ct FROM filtered WHERE owner_id IS NOT NULL GROUP BY owner_id ORDER BY ct DESC) x
    ),
    'by_modality', (
      SELECT COALESCE(jsonb_object_agg(coalesce(modality,'unknown'), ct), '{}'::jsonb)
      FROM (SELECT modality, COUNT(*) AS ct FROM filtered GROUP BY modality) x
    ),
    'by_program', (
      SELECT COALESCE(jsonb_agg(jsonb_build_object(
        'program_id', program_id, 'program_name', program_name, 'count', ct
      )), '[]'::jsonb)
      FROM (SELECT program_id, program_name, COUNT(*) AS ct FROM filtered GROUP BY program_id, program_name ORDER BY ct DESC) x
    ),
    'at_risk', (SELECT COUNT(*) FROM filtered WHERE health_score IN ('at_risk','critical'))
  );
$function$;

REVOKE ALL ON FUNCTION public.ecosystem_aggregates_v2(
  uuid, text, text, uuid, boolean, text, uuid, uuid, text, text
) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.ecosystem_aggregates_v2(
  uuid, text, text, uuid, boolean, text, uuid, uuid, text, text
) TO authenticated, service_role;

COMMENT ON FUNCTION public.ecosystem_aggregates_v2 IS
  'Ecosystem aggregate metrics over the full filtered dataset (independent of cursor pagination).';
