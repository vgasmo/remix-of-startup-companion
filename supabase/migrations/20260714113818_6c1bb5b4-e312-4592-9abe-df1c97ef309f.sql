...
BEGIN
  ...
  v_type := COALESCE(v_config->>'entity_type','lead');
  ...
  LOOP
    BEGIN
      ...
      IF v_source = 'phc' THEN
        ...
        IF NULLIF(v_n->>'phc_department','') IS NOT NULL THEN v_tags := v_tags || ('phc_department:'||(v_n->>'phc_department')); END IF;
        IF NULLIF(v_n->>'phc_building_hint','') IS NOT NULL THEN v_tags := v_tags || ('phc_building:'||(v_n->>'phc_building_hint')); END IF;
        ...
        IF NULLIF(v_n->>'hubspot_company_id_ref','') IS NOT NULL THEN v_external_ids := v_external_ids || jsonb_build_object('hubspot/company', v_n->>'hubspot_company_id_ref'); END IF;
        IF NULLIF(v_n->>'hubspot_deal_id_ref','') IS NOT NULL THEN v_external_ids := v_external_ids || jsonb_build_object('hubspot/deal', v_n->>'hubspot_deal_id_ref'); END IF;
        IF NULLIF(v_n->>'hubspot_contact_id_ref','') IS NOT NULL THEN v_external_ids := v_external_ids || jsonb_build_object('hubspot/contact', v_n->>'hubspot_contact_id_ref'); END IF;
        v_final_stage := COALESCE(v_config->>'default_stage','qualified');
      ELSE
      ...
      BEGIN
        ...
        UPDATE public.data_import_rows SET approval_state='committed', commit_result_json = v_rpc WHERE id = v_row.id;
