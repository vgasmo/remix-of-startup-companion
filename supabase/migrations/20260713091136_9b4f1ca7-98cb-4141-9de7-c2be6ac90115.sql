...
ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS mentor_monthly_target_hours integer NOT NULL DEFAULT 20;
...
BEGIN
  ...
  -- Sessions in workspaces where the mentor is an active member,
  -- scheduled in the window and considered "attended" (past or with notes/summary).
  ...
  -- Distinct startups (workspaces) the mentor is active in during the window.
  ...
  -- Explicit mentoring hours logged by the mentor in the window.
  ...
  -- Open follow-ups owned by mentor.
  SELECT COUNT(*)
    INTO v_open_followups
  FROM public.action_items ai
  WHERE ai.owner_user_id = p_mentor_id
    AND ai.status IN ('todo', 'in_progress', 'blocked');

  -- Feedback: only for sessions the mentor could have attended
  -- (i.e. sessions in workspaces where mentor is/was active).
...
REVOKE ALL ON FUNCTION public.get_mentor_impact(uuid, date, date) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_mentor_impact(uuid, date, date) TO authenticated, service_role;

COMMENT ON FUNCTION public.get_mentor_impact(uuid, date, date) IS
  'Returns mentor impact metrics (sessions, startups, hours, follow-ups, feedback avg). Callable by mentor themselves or admin.';
