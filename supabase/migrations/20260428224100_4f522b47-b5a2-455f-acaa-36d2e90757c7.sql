ALTER TABLE public.program_setup_drafts DROP CONSTRAINT IF EXISTS program_setup_drafts_status_check;

ALTER TABLE public.program_setup_drafts ADD CONSTRAINT program_setup_drafts_status_check
  CHECK (status = ANY (ARRAY['draft'::text, 'published'::text, 'discarded'::text, 'publish_failed'::text]));

ALTER TABLE public.program_setup_drafts ADD COLUMN IF NOT EXISTS last_publish_error text;
ALTER TABLE public.program_setup_drafts ADD COLUMN IF NOT EXISTS last_publish_failed_at timestamptz;

-- Update materializer to also pick up weeks whose gate_id is null but whose
-- week_number falls inside a gate's target_start_week..target_end_week range.
-- This closes the silent skip when the wizard publishes range-only weeks.
...
BEGIN
  ...
  LOOP
    ...
    -- Pick weeks explicitly assigned to this gate OR weeks with gate_id IS NULL
    -- whose week_number falls inside the gate's target range.
    FOR v_week IN
