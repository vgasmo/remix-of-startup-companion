...
-- Fixes four defects in public.log_completed_session_atomic:
--   1) Authorize BEFORE the idempotency lookup (info-disclosure fix).
--   2) Restrict authorization to staff, the workspace's assigned consultor,
--      or the caller being the declared primary consultant/mentor with an
--      accepted mentor_connection. Plain workspace members can only log a
--      session where they are the declared primary_consultant_id.
--   3) Validate every attendee user_id against the workspace (staff,
--      workspace member, accepted mentor, or assigned consultor).
--   4) Persist mentor attribution: add sessions.primary_mentor_id and write
--      it inside the INSERT.
...
BEGIN
  ...
  -- Idempotency check AFTER authorization to prevent info disclosure.
