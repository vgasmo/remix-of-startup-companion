...
BEGIN
  BEGIN
    ...
    -- E1: enqueue creates one attempt per active founder (2 of 3)
    ...
    -- E3: idempotent re-run inserts 0 (unique index blocks dup)
    ...
    -- E5: mark_delivered → delivered + delivered_at set
    ...
    -- E6: mark_failed retryable → failed_retryable + next_attempt_at bumped forward
    v_now := now();
    ...
    -- E7: expired lease is reclaimable
    -- Force a stale lease on attempt_b then claim again
    ...
    -- E8: max_attempts reached → failed_terminal
    UPDATE public.notification_attempts
    ...
    -- Rollback fixtures via sentinel
