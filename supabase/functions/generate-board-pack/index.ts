...
serve(async (req) => {
  ...
  try {
    ...
    // Resolve admin's preferred language
    const { data: adminProfile } = await supabase
      .from('profiles')
      .select('preferred_language')
      .eq('id', user.id)
      .maybeSingle();
    const locale: Locale = normalizeLocale(adminProfile?.preferred_language);
    ...
    const html = `<!DOCTYPE html><html lang="${locale}"><head><meta charset="utf-8"><title>${tb(locale, 'title')} — Startup Leiria</title>
<style>
  ...
  <div class="header">
    <h1>${tb(locale, 'title')}</h1>
    <p class="sub">${tb(locale, 'sub')} · ${reportDate}</p>
  </div>

  <h2>${tb(locale, 'mainKpis')}</h2>
  <div class="grid-4">
    <div class="kpi"><div class="kpi-value">${active}</div><div class="kpi-label">${tb(locale, 'activeStartups')}</div></div>
    <div class="kpi"><div class="kpi-value">${sessions30}</div><div class="kpi-label">${tb(locale, 'sessions')}</div></div>
    <div class="kpi"><div class="kpi-value">${milestones30}</div><div class="kpi-label">${tb(locale, 'milestonesDone')}</div></div>
    <div class="kpi"><div class="kpi-value">${kpis30}</div><div class="kpi-label">${tb(locale, 'kpisReported')}</div></div>
  </div>

  <h2>${tb(locale, 'stageDist')}</h2>
  ...
  <h2>${tb(locale, 'topHealthy')}</h2>
  <ul class="list">
    ${healthyTop.length ? healthyTop.map((w: any) => `<li><span>${w.startups?.name || w.id}</span><span class="pill pill-ok">${Math.round(w.health_score_numeric ?? 0)}</span></li>`).join('') : `<li style="color:#94a3b8;">${tb(locale, 'noData')}</li>`}
  </ul>

  <h2>${tb(locale, 'atRisk')}</h2>
  <ul class="list">
    ${atRisk.length ? atRisk.map((w: any) => `<li><span>${w.startups?.name || w.id}</span><span class="pill pill-risk">${w.health_status || tb(locale, 'riskLabel')}</span></li>`).join('') : `<li style="color:#94a3b8;">${tb(locale, 'noneCritical')}</li>`}
  </ul>

  <div class="footer">${tb(locale, 'footer')}</div>
</body></html>`;
