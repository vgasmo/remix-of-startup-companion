...
DECLARE
  ...
  v_cmd_a text := encode(digest(v_c1::text || '::v1::send', 'sha256'), 'hex');
  v_cmd_b text := encode(digest(v_c2::text || '::v1::send', 'sha256'), 'hex');
  v_cmd_c text := encode(digest(v_c3::text || '::v1::send', 'sha256'), 'hex');
  v_cmd_d1 text := encode(digest(v_c4::text || '::v1::send', 'sha256'), 'hex');
  v_cmd_d2 text := encode(digest(v_c4::text || '::v2::send', 'sha256'), 'hex');
  v_cmd_e text := encode(digest(v_c5::text || '::v1::send', 'sha256'), 'hex');
  v_cmd_f text := encode(digest(v_c6::text || '::v1::send', 'sha256'), 'hex');
  v_cmd_g text := encode(digest(v_c7::text || '::v1::send', 'sha256'), 'hex');
  v_cmd_h text := encode(digest(v_c8::text || '::v1::send', 'sha256'), 'hex');
  r jsonb;
  v_env_id text;
