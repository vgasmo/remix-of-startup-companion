...
DECLARE
  ...
  v_cmd_a text := md5(v_c1::text || '::v1::send');
  v_cmd_b text := md5(v_c2::text || '::v1::send');
  v_cmd_c text := md5(v_c3::text || '::v1::send');
  v_cmd_d1 text := md5(v_c4::text || '::v1::send');
  v_cmd_d2 text := md5(v_c4::text || '::v2::send');
  v_cmd_e text := md5(v_c5::text || '::v1::send');
  v_cmd_f text := md5(v_c6::text || '::v1::send');
  v_cmd_g text := md5(v_c7::text || '::v1::send');
  v_cmd_h text := md5(v_c8::text || '::v1::send');
  r jsonb;
  v_env_id text;
